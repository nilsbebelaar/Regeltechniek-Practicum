...
for zeta in zetas:
    T = np.array([natural_frequency ** 2])
    N = np.array([1.0, 2 * zeta *
                  natural_frequency, natural_frequency ** 2])
    H = control.tf(T, N)
    # print(H)

    y_punt.append(20 * np.log10(1 / (2 * zeta)))
    mag, phase, omega = control.bode(H, omega=np.logspace(1.5, 2.5, 10000),label=f"$\zeta$={zeta}")
